...
def retrain_classifier_model(model_file, x_train, y_train, epochs, batch_sizes, model_suffix):
    ...
    model.compile(loss=MeanSquaredError(), optimizer=tensorflow.keras.optimizers.Adam(), metrics=['mse', "acc"])
    file_path = "../dataset/adult/retrain/{}.h5".format(model_suffix)
    C_point = ModelCheckpoint(file_path, monitor='mse', save_best_only=True)
    model.fit(x=x_train, y=y_train, epochs=epochs, batch_size=batch_sizes, verbose=1, callbacks=C_point)
...
if __name__ == "__main__":
    train_file = "../dataset/adult/data/train.npz.npy"
    cluster_file = "../dataset/adult/test/retrain_avg_clusters.npz.npy"
    BL_model_file = "../dataset/adult/model/BL.h5"
    for cos in [0.70, 0.75, 0.80, 0.85, 0.90, 0.95]:
        for i in range(Retrain_size):
            for m in ["RobustFair"]:
                generate_file = "../dataset/adult/test/Train_{}_{}_10_10.npz.npy".format(m, i)
